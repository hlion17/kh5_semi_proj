...
public class RecipeServiceImpl implements RecipeService {
	...
	public List<Recipe> recipeBoard() {
		...
		//게시글 전체 조회 결과 반환
		return recipeDao.selectAll( JDBCTemplate.getConnection() );
	}
	...
	public List<Recipe> recipeBoard(Paging paging) {
		...
		//페이징 적용해서 조회 결과 반환
		return recipeDao.selectAll( JDBCTemplate.getConnection(), paging );
	}
	...
	public Recipe getRecipeIdx(HttpServletRequest req) {
		...
		//전달파라미터 recipeIdx를 저장할 DTO객체 생성
		Recipe recipeIdx = new Recipe();
		
		String param = req.getParameter("recipeIdx");
		if( param != null && !"".equals( param ) ) {
			recipeIdx.setRecipeIdx( Integer.parseInt(param) );
		} else {
			System.out.println("[WARN] RecipeService getRecipeIdx() - recipeIdx값이 null이거나 비어있음");
		}
	...
	public Recipe recipeContent(Recipe recipeIdx) {
		...
		//조회수 증가
		if( recipeDao.updateHit(conn, recipeIdx) > 0 ) {
		...
		//게시글 조회
		Recipe recipe = recipeDao.selectRecipeByRecipeIdx(conn, recipeIdx);
	...
	public void recipeInsert(HttpServletRequest req) {
		...
		while( iter.hasNext() ) {
			...
			if( item.isFormField() ) {
				...
				if( "title".equals(key) ) {
					recipe.setRecipeTitle(value);
					
				} else if ( "content".equals(key) ) {
					recipe.setRecipeExplain(value);
