...
public class Paging {
	...
	//디폴트생성자 추가하는게 데이터 관리 차원에서 좋다
	//-> 페이징 로직이 완성되지 않는다
	public Paging() { }
	
	
	//생성자 이용해서 매개변수를 두개 받았을 때, makePaging메소드 호출
	//-> 전달받은 값에 따라서 페이징 계산 
	...
	//totalCount만 제어하는 상황
	//	curpage(전달값)가 없을 때 기본값으로 1번페이지로 설정
	//-> makePaging 메소드 호출되면서 setCurPage(1) 설정되어있음
	...
	private void makePaging() {
		...
		//curPage가 0일 경우 curpege를 1번으로 설정한다
		//첫 페이지를 기본 페이지로 설정한다
		if(curPage==0) setCurPage(1);
		
		//listcount가 설정이 안되었을 때 객체가 만들어지면 listcount를 10개로 설정
		//->화면에 보여질 게시글 수를 10개로 설정한다
		if(listCount==0) setListCount(10);
		
		//화면에 보여질 페이징 수를 10개로 설정한다
